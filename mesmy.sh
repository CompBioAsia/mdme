...
for RUN in step1 step2 step3 step4 step5 ; do
 ...
 if [[ ! -f $RUN.rst7 ]]; then
     ...
     pmemd.cuda -O -i $RUN.in -p $prmtop_file -c $inpcrd_file -ref $inpcrd_file -o $RUN.out -x $RUN.nc -r $RUN.rst7 -inf $RUN.mdinfo
 else
...
for RUN in step6 step7 step8 step9 ; do
 ...
 if [[ ! -f $RUN.rst7 ]]; then
     echo "File -- $RUN.rst7 -- does not exists. Running job..."
     pmemd.cuda -O -i $RUN.in -p $prmtop_file -c $inpcrd_file -ref $REF -o $RUN.out -x $RUN.nc -r $RUN.rst7 -inf $RUN.mdinfo
 else
     echo "File -- $RUN.rst7 -- exists.  Checking the next step."
 fi
  echo ""
